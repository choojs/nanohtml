  ...
  var hx = hyperx(function (tag, props, children) {
    ...
    Object.keys(props).forEach(function (key) {
      if (key === props[key]) {
        ...
        spread.push(unExpr(key))
      } else if (isExpr(key)) {
        // expr props <div ${prop}=${val}}>
        spread.push('{[' + unExpr(key) + ']:' + unExpr(props[key]) + '}')
      } else {
        sprops.push(JSON.stringify(key) + ':' + unExpr(props[key]))
      }
    })

    sprops = '{' + sprops.join(',') + '}'

    return expr(h + '.createElement(' + unExpr(tag) +
      ',' + (spread.length ? ('Object.assign(' + spread.join(',') + ',' + sprops + ')') : sprops) +
      (children && children.length ? (',[' + children.map(unExpr).join(',') + ']') : '') +
      ')')
